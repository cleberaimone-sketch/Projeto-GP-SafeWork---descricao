    ...
    <div className="flex flex-col h-[520px] bg-white rounded-xl border border-slate-200">
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {msgs.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-slate-500 text-sm">Olá! Sou o Dieguito, especialista em engenharia de segurança.</p>
            <p className="text-slate-500 text-xs mt-1">Pergunte sobre GHE, EPIs, laudos, PGR ou conformidade NR</p>
          </div>
        )}
        {msgs.map((m, i) => (
      ...
      <div className="p-3 border-t border-slate-200 flex gap-2">
        <input
          ...
          className="flex-1 bg-slate-100 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 outline-none focus:ring-1 focus:ring-orange-500"
          disabled={loading}
        />
        <button
          onClick={send}
