...
import {
  ...
  CUSTO_2026_PLANILHA_MENSAL, CUSTO_2025_PLANILHA_MENSAL,
  CUSTO_2025_PLANILHA_TOTAL,
  MEDIA_SALARIAL_2025,
  // 2026 (ano corrente) — planilha nova
  INDICADORES_DP_2026, TAXA_TURNOVER_2026, MEDIA_SALARIAL_2026,
  CUSTO_2026_PLANILHA_TOTAL,
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <div className="max-w-screen-2xl mx-auto px-6 md:px-8 py-6 md:py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
          ...
          <div className="bg-white rounded-xl p-4 border border-slate-200">
            <div className="flex items-baseline gap-2 mb-1">
              <p className="text-xl font-bold text-emerald-700 tabular-nums">{INDICADORES_DP_2026.contratacoes}</p>
              <p className="text-xl font-bold text-red-700 tabular-nums">{INDICADORES_DP_2026.desligamentos}</p>
            </div>
            <p className="text-[11px] text-slate-500 uppercase tracking-wider font-medium">Admissões / Deslig.</p>
            <p className="text-[10px] text-slate-400 mt-0.5">Jan-Jun 2026</p>
          </div>

          <div className={`rounded-xl p-4 border ${TAXA_TURNOVER_2026 > 5 ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-200'}`}>
            <p className={`text-xl font-bold tabular-nums mb-1 ${TAXA_TURNOVER_2026 > 5 ? 'text-amber-800' : 'text-slate-900'}`}>{TAXA_TURNOVER_2026.toFixed(1)}%</p>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="text-right px-3 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider">2025 (Jan-Nov)</th>
                    <th className="text-right px-3 py-2 text-[10px] font-bold text-teal-700 uppercase tracking-wider">2026 (Jan-Jun)</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  <tr>
                    ...
                    <td className="px-3 py-2 text-right tabular-nums">{INDICADORES_DP.headcountFinal}</td>
                    <td className="px-3 py-2 text-right tabular-nums font-semibold text-teal-800">{INDICADORES_DP_2026.headcountFinal}</td>
                    <td className={`px-3 py-2 text-right tabular-nums text-[11px] font-semibold ${INDICADORES_DP_2026.headcountFinal >= INDICADORES_DP.headcountFinal ? 'text-emerald-700' : 'text-red-700'}`}>
                      {INDICADORES_DP_2026.headcountFinal - INDICADORES_DP.headcountFinal >= 0 ? '+' : ''}{INDICADORES_DP_2026.headcountFinal - INDICADORES_DP.headcountFinal}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 text-slate-700 font-medium">Contratações</td>
                    <td className="px-3 py-2 text-right tabular-nums">{INDICADORES_DP.contratacoes}</td>
                    <td className="px-3 py-2 text-right tabular-nums font-semibold text-teal-800">{INDICADORES_DP_2026.contratacoes}</td>
                    <td className="px-3 py-2 text-right tabular-nums text-[11px] font-semibold text-slate-500">
                      {INDICADORES_DP_2026.contratacoes - INDICADORES_DP.contratacoes >= 0 ? '+' : ''}{INDICADORES_DP_2026.contratacoes - INDICADORES_DP.contratacoes}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 text-slate-700 font-medium">Desligamentos</td>
                    <td className="px-3 py-2 text-right tabular-nums">{INDICADORES_DP.desligamentos}</td>
                    <td className="px-3 py-2 text-right tabular-nums font-semibold text-teal-800">{INDICADORES_DP_2026.desligamentos}</td>
                    <td className="px-3 py-2 text-right tabular-nums text-[11px] font-semibold text-slate-500">
                      {INDICADORES_DP_2026.desligamentos - INDICADORES_DP.desligamentos >= 0 ? '+' : ''}{INDICADORES_DP_2026.desligamentos - INDICADORES_DP.desligamentos}
                    </td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 text-slate-700 font-medium">Turnover acumulado</td>
                    <td className="px-3 py-2 text-right tabular-nums">{TAXA_TURNOVER.toFixed(1)}%</td>
                    <td className="px-3 py-2 text-right tabular-nums font-semibold text-teal-800">{TAXA_TURNOVER_2026.toFixed(1)}%</td>
                    <td className={`px-3 py-2 text-right tabular-nums text-[11px] font-semibold ${TAXA_TURNOVER_2026 <= TAXA_TURNOVER ? 'text-emerald-700' : 'text-red-700'}`}>
                      {(TAXA_TURNOVER_2026 - TAXA_TURNOVER).toFixed(1)}pp
                    </td>
                  </tr>
                  <tr className="bg-teal-50/40">
                    <td className="px-3 py-2 text-slate-700 font-medium">CTSE Total</td>
                    <td className="px-3 py-2 text-right tabular-nums">{fmtReal(CUSTO_2025_PLANILHA_TOTAL)}</td>
                    <td className="px-3 py-2 text-right tabular-nums font-semibold text-teal-800">{fmtReal(CUSTO_2026_PLANILHA_TOTAL)}</td>
                    <td className="px-3 py-2 text-right tabular-nums text-[11px] font-semibold text-slate-500">períodos ≠</td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 text-slate-700 font-medium">Média Salarial</td>
                    <td className="px-3 py-2 text-right tabular-nums">{fmtReal(MEDIA_SALARIAL_2025)}</td>
                    <td className="px-3 py-2 text-right tabular-nums font-semibold text-teal-800">{fmtReal(MEDIA_SALARIAL_2026)}</td>
                    <td className={`px-3 py-2 text-right tabular-nums text-[11px] font-semibold ${MEDIA_SALARIAL_2026 >= MEDIA_SALARIAL_2025 ? 'text-emerald-700' : 'text-red-700'}`}>
                      {MEDIA_SALARIAL_2026 >= MEDIA_SALARIAL_2025 ? '+' : ''}{(((MEDIA_SALARIAL_2026 - MEDIA_SALARIAL_2025) / MEDIA_SALARIAL_2025) * 100).toFixed(1)}%
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        {/* Custo por Unidade e por Vínculo — Planilha 2026 */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
            ...
            <div className="space-y-3">
              {CUSTO_2026_POR_VINCULO.map(v => {
                const pct = (v.total / CUSTO_2026_POR_UNIDADE_TOTAL) * 100
            ...
            <p className="text-[10px] text-slate-400 mt-4 leading-relaxed">
              PJ representa <strong className="text-slate-600">{((CUSTO_2026_POR_VINCULO[1].total / CUSTO_2026_POR_UNIDADE_TOTAL) * 100).toFixed(0)}%</strong> do custo total —
        ...
        <div className="mb-8">
          <CtseHistorico
            meses={MESES_RH}
            ctseAtual={CUSTO_2026_PLANILHA_MENSAL}
            ctseAnterior={CUSTO_2025_PLANILHA_MENSAL}
