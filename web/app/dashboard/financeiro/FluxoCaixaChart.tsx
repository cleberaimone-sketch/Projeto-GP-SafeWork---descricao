...
export default function FluxoCaixaChart({ porMes, buckets90d, saldoAtual }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <h2 className="text-sm font-semibold text-slate-900 mb-1">Previsão — Próximos 90 Dias</h2>
        <p className="text-xs text-slate-500 mb-4">Lançamentos pendentes e vencidos agrupados por prazo</p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {buckets90d.map((b, i) => (
            <div key={i} className={`rounded-xl border p-4 ${b.saldo_liquido >= 0 ? 'border-green-900/50 bg-green-950/20' : 'border-red-200 bg-red-50'}`}>
              <p className="text-xs font-semibold text-slate-500 mb-3">{b.label}</p>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-emerald-700">A receber</span>
                  <div className="text-right">
                    <p className="text-sm font-semibold text-emerald-800">{fmt(b.a_receber)}</p>
                    <p className="text-[10px] text-slate-500">{b.qtd_receber} títulos</p>
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-red-700">A pagar</span>
                  <div className="text-right">
                    <p className="text-sm font-semibold text-red-800">{fmt(b.a_pagar)}</p>
                    <p className="text-[10px] text-slate-500">{b.qtd_pagar} títulos</p>
                  </div>
                </div>
                <div className="h-px bg-slate-200 my-1" />
                <div className="flex justify-between items-center">
                  <span className="text-xs text-slate-500">Saldo líquido</span>
                  <p className={`text-sm font-bold ${b.saldo_liquido >= 0 ? 'text-emerald-800' : 'text-red-800'}`}>
        ...
        {buckets90d.length === 0 && (
          <p className="text-xs text-slate-500">Nenhum lançamento pendente nos próximos 90 dias</p>
        )}
      </div>
    </div>
  )
