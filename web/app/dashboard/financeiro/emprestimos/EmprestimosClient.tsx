...
function Card({ label, valor, cor, sub }: {
  ...
  const c = { red: 'text-red-700', amber: 'text-amber-700', slate: 'text-slate-800', violet: 'text-violet-700' }[cor]
  return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200">
          <h3 className="text-sm font-semibold text-slate-900">Posição por Tipo</h3>
              ...
              <div key={t.tipo}>
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    {t.qtdAberto > 0 && <span className="text-[10px] text-slate-400">({t.qtdAberto})</span>}
                  </div>
                  <div className="flex items-center gap-2">
                ...
                {t.pago > 0 && (
                  <p className="text-[10px] text-slate-400 mt-1">Já pago histórico: <span className="text-slate-500 tabular-nums">{fmt(t.pago)}</span></p>
                )}
              </div>
            )
          })}
        ...
        <div className="border-t border-slate-200 p-4 grid grid-cols-2 gap-3 text-[10px]">
          <div>
            <p className="text-slate-500 uppercase tracking-wider mb-1">Histórico</p>
            <div className="flex justify-between"><span className="text-slate-500">Total recebido:</span><span className="text-emerald-700 tabular-nums">{fmt(kpis.totalRecebidoHist)}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Total pago:</span><span className="text-red-700 tabular-nums">{fmt(kpis.totalPagoHistorico)}</span></div>
            <div className="flex justify-between font-bold border-t border-slate-200 mt-1 pt-1">
              <span className="text-slate-500">Saldo histórico:</span>
              <span className={`tabular-nums ${(kpis.totalRecebidoHist - kpis.totalPagoHistorico) >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
          ...
          <div>
            ...
            <div className="flex justify-between"><span className="text-slate-500">A pagar:</span><span className="text-red-700 tabular-nums">{fmt(kpis.saldoAberto)}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">A receber:</span><span className="text-emerald-700 tabular-nums">{fmt(kpis.aReceber)}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Juros:</span><span className="text-red-700/70 tabular-nums">{fmt(kpis.jurosAberto)}</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200">
          <h3 className="text-sm font-semibold text-slate-900">Posição por Empresa</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
                ...
                return (
                  <tr key={i} className="border-t border-slate-200 hover:bg-slate-100/30">
                    ...
                    <td className="px-2 py-2 text-right text-red-700 tabular-nums font-medium">
                      {fmt(e.aberto)}<span className="text-[9px] text-slate-400 ml-1">{pct.toFixed(0)}%</span>
                    </td>
              ...
              {empresas.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-400">Nenhuma empresa com empréstimos em aberto</td></tr>
              )}
            </tbody>
          </table>
        </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="p-4 border-b border-slate-200 flex items-center justify-between flex-wrap gap-3">
        <div>
          <h3 className="text-sm font-semibold text-slate-900">Cronograma — Próximos 12 meses</h3>
          <p className="text-[10px] text-slate-500 mt-0.5">
            Total a pagar: <span className="text-red-700 font-medium">{fmt(totalProximos12)}</span> · Mês mais pesado: <span className="text-amber-700 font-medium">{maiorMes.nomeMes} ({fmt(maiorMes.total)})</span>
          </p>
        </div>
      </div>
      <div className="p-4 space-y-3">
        {meses.map(m => {
          ...
          return (
            <div key={m.mesKey}>
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-700 capitalize w-16">{m.nomeMes}</span>
                  {m.qtd > 0 && <span className="text-[10px] text-slate-400">({m.qtd} parc.)</span>}
                </div>
                <div className="flex items-center gap-3 text-xs tabular-nums">
                  {m.juros > 0 && <span className="text-red-700/70" title="Juros">+{fmt(m.juros)} j</span>}
                  <span className="text-red-700 font-medium">{fmt(m.total)}</span>
                </div>
              </div>
              ...
              {m.total > 0 && <div className="text-[9px] text-slate-400 mt-0.5">{pct.toFixed(0)}% do maior mês</div>}
            </div>
          )
        })}
      </div>
...
function AbaHistorico({ meses }: { meses: MesHistorico[] }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="p-4 border-b border-slate-200">
        <h3 className="text-sm font-semibold text-slate-900">Histórico 12 meses — Entradas vs Saídas</h3>
        <p className="text-[10px] text-slate-500 mt-0.5">
          <span className="text-emerald-700">Verde</span> = empréstimos recebidos ·
          <span className="text-red-700 ml-1">Vermelho</span> = principal pago ·
          <span className="text-red-800 ml-1">Vinho</span> = juros pagos
        </p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {meses.map(m => {
              ...
              return (
                <tr key={m.mesKey} className="border-t border-slate-200 hover:bg-slate-100/30">
                  <td className="px-4 py-2 text-slate-700 capitalize">{m.nomeMes}</td>
                  <td className="px-2 py-2 text-right text-emerald-700 tabular-nums">{m.entradas > 0 ? fmt(m.entradas) : '—'}</td>
                  <td className="px-2 py-2 text-right text-red-700 tabular-nums">{m.saidas > 0 ? fmt(m.saidas) : '—'}</td>
                  <td className="px-2 py-2 text-right text-red-800 tabular-nums">{m.juros > 0 ? fmt(m.juros) : '—'}</td>
                  <td className={`px-4 py-2 text-right tabular-nums font-medium ${m.saldoLiquido >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>{fmt(m.saldoLiquido)}</td>
    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="p-4 border-b border-slate-200 flex items-center justify-between flex-wrap gap-3">
          ...
          {(['aberto', 'pago', 'todos'] as const).map(s => (
            <button
              ...
              className={`px-3 py-1 text-xs rounded-md transition-colors ${
                filtroStatus === s ? 'bg-slate-700 text-white' : 'bg-slate-50 text-slate-500 hover:text-slate-700'
              }`}
            >
        ...
        <input
          ...
          className="flex-1 min-w-[200px] bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-800 placeholder-slate-600"
        />
      ...
      <div className="overflow-x-auto max-h-[600px]">
        <table className="w-full text-xs">
          ...
          <tbody>
              ...
              <tr key={l.id} className="border-t border-slate-200 hover:bg-slate-100/30">
                ...
                <td className="px-2 py-2">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-slate-700">{l.tipoEmpLabel}</span>
                  </span>
                </td>
                <td className="px-2 py-2 text-slate-500 truncate max-w-[100px]">{l.empresa_nome}</td>
                <td className="px-2 py-2 text-slate-800 truncate max-w-[260px]">{l.descricao}</td>
                <td className="px-2 py-2">
                  <span className={`text-[9px] px-1.5 py-0.5 rounded uppercase font-medium ${
                    l.status === 'pago'     ? 'bg-emerald-950/60 text-emerald-700' :
                    l.status === 'parcial'  ? 'bg-amber-950/60 text-amber-700'     :
                    l.status === 'vencido'  ? 'bg-red-950/60 text-red-700'         :
            ...
            {filtrados.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">Nenhum lançamento encontrado</td></tr>
            )}
          </tbody>
        </table>
      </div>
